...
export default function AdminSettingsPage() {
    ...
    fetch("/api/admin/settings")
      ...
      .then((body) => {
        ...
        setForm({
          ...
          vpnEnabled: s.vpnEnabled === true,
          vpnOvpnConfig: s.vpnOvpnConfig || ""
        });
  ...
  // Repère visuel VPN : interroge le statut du conteneur morss (voir
  // /api/admin/vpn-status). "reachable: false" couvre aussi bien "morss pas
  // encore redéployé avec ce module" que "conteneur en cours de démarrage" —
  // pas la peine de distinguer les deux ici, le message reste le même.
  async function loadVpnStatus() {
    ...
    try {
      ...
      setVpnStatus({
        ...
        vpnEnabled: body.vpnEnabled === true,
        connected: body.connected === true
      });
    } catch {
      setVpnStatus({ reachable: false, vpnEnabled: false, connected: false });
    } finally {
  ...
  function payload() {
    return {
      ...
      vpnOvpnConfig: form.vpnOvpnConfig
    };
  }
    ...
    <main className="paper-panel mx-auto w-full lg:w-3/4 rounded-sm px-6 py-10 shadow-[0_10px_60px_-15px_rgba(26,26,26,0.35)] ring-1 ring-ink/10 md:px-10 md:py-14">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          <fieldset className="space-y-3 border-t-2 border-ink pt-4">
            <legend className="mb-1 font-display text-xs uppercase tracking-[0.2em]">
              VPN (module morss intégré)
            </legend>
            <p className="text-xs italic text-sepia">
              Morss (intégré au même déploiement que DailySpoon, voir docker-compose) sert de repli
              automatique quand la lecture directe d&apos;un article ou d&apos;un flux échoue (403,
              blocage anti-bot — Reddit, korben.info, Cloudflare...) : la requête repart depuis morss
              plutôt que directement depuis ce serveur. Morss va chercher les flux/articles depuis sa
              propre IP — potentiellement bloquée aussi, sur les mêmes sites. Active ce VPN pour que
              morss sorte par une autre IP. N&apos;affecte QUE morss, jamais DailySpoon lui-même
              (accès à l&apos;appli inchangé). Colle ici le contenu complet d&apos;un fichier .ovpn
              fourni par ton fournisseur VPN.
            </p>
            <label className="flex items-center gap-2 text-xs italic text-sepia">
              <input
                type="checkbox"
                checked={form.vpnEnabled}
                onChange={(e) => set("vpnEnabled", e.target.checked)}
                className="accent-journal"
              />
              Activer le VPN pour morss
            </label>
            <label className="block">
              <span className="mb-1 block text-xs uppercase tracking-[0.15em] text-neutral-600">
                Fichier .ovpn (contenu complet)
              </span>
              <div className="mb-2 flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    reader.onload = () => {
                      set("vpnOvpnConfig", String(reader.result || ""));
                    };
                    reader.readAsText(file);
                    // Permet de recharger le même fichier une seconde fois si besoin.
                    e.target.value = "";
                  }}
                  className="text-xs file:mr-3 file:border file:border-ink/40 file:bg-paper file:px-3 file:py-1.5 file:text-xs file:uppercase file:tracking-[0.1em] file:text-journal hover:file:bg-ink/5"
                />
                <span className="text-xs italic text-sepia">
                  ou colle le contenu directement ci-dessous
                </span>
              </div>
              <textarea
                value={form.vpnOvpnConfig}
                onChange={(e) => set("vpnOvpnConfig", e.target.value)}
                rows={8}
                placeholder={"client\nproto tcp-client\nremote ...\n<ca>\n...\n</ca>\n..."}
                className="w-full border border-ink/40 bg-paper px-3 py-2 font-mono text-xs focus:outline-none focus:ring-1 focus:ring-ink"
              />
            </label>
            <p className="text-xs italic text-sepia">
              Après avoir enregistré (ou changé) ce réglage, redémarre le conteneur <code>morss</code>{" "}
              depuis Coolify pour qu&apos;il en tienne compte — un simple enregistrement ici ne suffit
              pas, morss ne relit ce réglage qu&apos;à son démarrage.
            </p>
            <div className="flex flex-wrap items-center gap-3 pt-1">
              {/* Vert = vraiment connecté, rouge = activé mais tombé (anomalie
                  à surveiller), gris = état normal "pas concerné" (désactivé,
                  ou morss pas encore joignable) — pour que la couleur suive
                  l'intuition habituelle plutôt que la palette journal/sepia
                  utilisée ailleurs sur cette page. */}
              <span
                className={`inline-flex items-center gap-2 border px-3 py-1.5 text-xs uppercase tracking-[0.1em] ${
                  vpnStatus?.connected
                    ? "border-green-700 text-green-700"
                    : vpnStatus?.reachable && vpnStatus?.vpnEnabled
                      ? "border-red-700 text-red-700"
                      : "border-ink/20 text-neutral-500"
                }`}
              >
                <span
                ...
                {!vpnStatus
                  ...
                  : !vpnStatus.reachable
                    ...
                    : vpnStatus.connected
                      ? "VPN connecté"
                      : vpnStatus.vpnEnabled
                        ? "VPN activé, tunnel non monté"
                        : "VPN désactivé"}
              </span>
              <button
                type="button"
                onClick={loadVpnStatus}
                disabled={loadingVpnStatus}
                className="border border-ink/40 px-3 py-1.5 text-xs uppercase tracking-[0.15em] text-journal hover:bg-ink/5 disabled:opacity-50"
              >
                {loadingVpnStatus ? "Vérification..." : "Vérifier"}
              </button>
            </div>
          </fieldset>
