...
// de page, encarts métadonnées "Franchise :"/"Réalisateur :"...) qu'un
// article extrait via Readability peut encore contenir. Historiquement
// défini directement dans /api/article-proxy (modale de lecture) ; factorisé
// ici pour que ce soit un seul endroit à faire évoluer — voir deepTrimJunk,
// le seul point d'entrée utilisé par /api/article-proxy.
...
// Encarts "métadonnées" façon fiche produit/vidéo (franchise, plateformes,
// réalisateur, année de production...) — vus en usage réel sur gamekult.com
// (bloc "Minecraft Construction, Jeu de survie 18 novembre 2011 Franchise :
// Minecraft", 76 caractères, largement au-dessus du seuil de 40) et
// cnrs.fr/Le journal ("À propos de cette vidéo", "Titre original :",
// "Réalisateur :"...). Sans cette liste, un paragraphe comme celui de
// Gamekult passait le test de longueur et le test de ratio de liens haut la
// main, et se retrouvait donc classé "vrai contenu" — ce qui en faisait un
// point d'ancrage erroné : tout ce qui était coincé ENTRE ce faux "vrai
// paragraphe" et le VRAI premier paragraphe de l'article (ex. la liste de
// plateformes) n'était alors jamais retiré, ni par trimLeadingJunk (qui ne
// regarde qu'AVANT le premier vrai paragraphe) ni par trimTrailingJunk (qui
// ne regarde qu'APRÈS le dernier).
export const METADATA_LABEL_PHRASES = [
  "franchise :",
  "titre original",
  "année de production",
  "réalisateur :",
...
export function isSubstantiveParagraph(el: Element): boolean {
  ...
  if (METADATA_LABEL_PHRASES.some((p) => lower.includes(p))) return false;
...
// trimTrailingJunk/trimLeadingJunk ne regardent que les enfants DIRECTS de
// l'élément reçu — insuffisant quand un site regroupe TOUT (vrai texte ET
// chrome) sous un seul conteneur wrapper, lui-même seul enfant pertinent de
// root (vu en usage réel sur cnrs.fr/Le journal : bandeau "Vous êtes ici" +
// tout l'encart "À propos de cette vidéo" imbriqués avec l'unique vrai
// paragraphe sous un même bloc). Plutôt que de parcourir TOUT l'arbre sans
// discernement (risque de vider aussi un encart voisin — bio auteur,
// suggestions... — qui contiendrait son propre paragraphe légitime), on ne
// descend QUE le long de la "colonne vertébrale" du contenu réel : tant que
// le contenu substantiel est concentré dans un SEUL enfant du niveau
// courant, on réapplique les deux passes à CE niveau plus profond ; dès que
// plusieurs enfants contiennent du texte réel (donc qu'on est déjà au bon
// niveau, celui où le chrome et le vrai texte sont de vrais frères), on
// s'arrête. Profondeur bornée par sécurité (MAX_SPINE_DEPTH).
const MAX_SPINE_DEPTH = 4;

export function deepTrimJunk(root: Element): void {
  let current: Element = root;
  for (let depth = 0; depth < MAX_SPINE_DEPTH; depth++) {
    trimTrailingJunk(current);
    trimLeadingJunk(current);

    const withReal = Array.from(current.children).filter((c) => containsSubstantiveParagraph(c));
    if (withReal.length !== 1) break;

    const next = withReal[0];
