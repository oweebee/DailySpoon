...
const WRITING_STYLES: Record<string, string> = {
  ...
  // aux faits.
  // "JMLP" : imitation de la FORME oratoire (registre, syntaxe, figures de
  // style, cadence) décrite par l'utilisateur — explicitement PAS de ses
  // idées ou de son contenu politique. Garde-fous renforcés en conséquence :
  // aucune opinion politique réelle (immigration, nationalisme, etc.),
  // aucune citation inventée présentée comme authentique, jamais de propos
  // raciste/antisémite/xénophobe ni de généralisation déshumanisante sur un
  // groupe réel, jamais d'incitation à la haine/violence — seule la
  // mécanique rhétorique (comment c'est dit) s'applique au contenu factuel
  // de l'article, jamais le fond idéologique du personnage réel.
  jmlp: `Consigne de TON impérative pour "headline" ET "summary" (et "frontPageSummary" le cas échéant) : adopte UNIQUEMENT la forme oratoire suivante, jamais le fond idéologique d'une personne réelle — une prose solennelle, ample et très construite, alternant longues périodes rhétoriques et phrases courtes percutantes. Ton assuré, parfois martial, avec une impression constante de maîtrise. Chaque paragraphe progresse vers une conclusion frappante, souvent formulée comme une maxime ou une sentence. Vocabulaire soutenu, précis et volontiers littéraire, enrichi de références historiques, culturelles ou classiques quand c'est pertinent pour le sujet traité. Répétitions, anaphores, accumulations et oppositions (antithèses) rythment le texte ; des questions rhétoriques interpellent directement le lecteur ("Que constate-t-on ?", "Voyez-vous..."). Une pointe d'ironie ou de trait d'esprit peut renforcer l'impact d'une phrase, sans jamais tomber dans l'insulte. Registre très soutenu et classique ; rythme lent et cadencé, avec des effets de respiration ; structure en progression logique vers une conclusion marquante ; ton affirmatif et théâtral. Adresse directe possible au lecteur ("Mes amis", "Voyez-vous"). L'objectif est de convaincre autant par la forme que par le fond, en donnant au texte une forte puissance mémorielle (formules ciselées, aphorismes) — mais le "fond" reste EXCLUSIVEMENT les faits de l'article traité : jamais d'opinion politique réelle (immigration, nation, identité, etc.), jamais de citation inventée présentée comme authentique, jamais de propos raciste, antisémite, xénophobe ou de généralisation déshumanisante visant un groupe réel, jamais d'incitation à la haine ou à la violence, jamais de propos hors-sujet par rapport à l'article. Le fond reste STRICTEMENT fidèle aux faits rapportés — seule la mécanique rhétorique (comment c'est dit) change, jamais ce qui est défendu politiquement. Le titre peut être plus solennel/frappant que d'habitude, tant qu'il reste fidèle au sujet.`,
