 ...
 *
 *  L'option ":get=page" (voir cgi_get dans morss/wsgi.py) est INDISPENSABLE
 *  ici : sans elle, morss essaie d'interpréter l'URL comme un FLUX à parser
 *  plutôt que comme une page à nettoyer, et rejette toute page d'article
 *  normale avec "Link provided is not a valid feed" — c'était la VRAIE cause
 *  du "Korben (entre autres) se retrouve sans image malgré extractFirstImageSrc
 *  + repli og:image" (voir le diagnostic temporaire plus haut dans ce
 *  fichier), pas un souci de blocage réseau/VPN comme on le pensait alors.
 */
async function fetchPageHtmlWithMorssFallback(url: string): Promise<string | null> {
  const direct = await fetchHtmlOnce(url);
  if (direct) return direct;

  if (isAlreadyMorssUrl(url, MORSS_BASE_URL)) return null;

  const strippedUrl = url.replace(/^https?:\/\//, "");
  const viaMorss = await fetchHtmlOnce(`${MORSS_BASE_URL}/:get=page/${strippedUrl}`);
  if (!viaMorss) {
    await writeLog(
      "warn",
      "custom-feeds",
